...
namespace BPA.AppConfiguration.Controllers
{
    ...
    public class LOBController : BaseController<LOBController>
    {
        ...
        [HttpGet]
        [Route("GetLOBList")]
        public async Task<MessageResponse<List<LOBModel>>> GetLOBList([FromQuery] string? LOBName)
        {
            var result = new MessageResponse<List<LOBModel>>();
            List<LOBModel> oLOBModel = new List<LOBModel>();

            try
            {
                var LOBList = await Task.Run(() =>
                _repositoryLOB.GetLOBList(Encoder.HtmlEncode(LOBName), false, base.oTenant));
                oLOBModel = LOBList.Select(x => new LOBModel
                {
                    LOBID = x.iLOBID,
                    LOBName = x.sLOBName,
