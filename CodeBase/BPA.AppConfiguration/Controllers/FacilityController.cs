...
namespace BPA.AppConfiguration.Controllers
{
    ...
    public class FacilityController : BaseController<FacilityController>
    {
        ...
        [ProducesResponseType(typeof(List<FacilityModel>), StatusCodes.Status200OK)]
        [HttpGet]
        [Route("GetFacilityList")]
        public async Task<MessageResponse<List<FacilityModel>>> GetFacilityList([FromQuery] string? searchFacility)
        {
            var result = new MessageResponse<List<FacilityModel>>();
            List<FacilityModel> lstFacilityview = new List<FacilityModel>();

            try
            {
                var dbresult = await Task.Run(() => _repositoryFacility.GetFacilityList(searchFacility == null ? "" : searchFacility.Trim(), false, base.oTenant));

                lstFacilityview = dbresult.Select(x => new FacilityModel
                {
                    FacilityID = x.iFacilityID,
                    FacilityName = x.sFacilityName,
                    FacilityDescription = x.sFacilityDescription,
                    Disabled = x.bDisabled,
                    LocationID = x.iLocationID
                }).ToList();
                result.Data = lstFacilityview;
                result.TotalCount = lstFacilityview.Count;
            }
            catch (Exception ex)
            {
                result.Message = _logger.Error(new LoggerInfo() { actionName = MethodBase.GetCurrentMethod().Name, message = $"{ex?.Message} {ex?.InnerException?.Message} {ex?.StackTrace}", requestId = CorrelationId });
