...
namespace BPA.AppConfiguration.Controllers
{
    ...
    public class LanguageConfigController : BaseController<LanguageConfigController>
    {
        ...
        public ActionResult<MessageResponse<IList<BEMailTranslatorConfiguration>>> GetCampaignWiseLangList(int iStoreID, int iFormId = 1)
        {
            var result = new MessageResponse<IList<BEMailTranslatorConfiguration>>();
            try
            {
                var value = _languagesService.GetCampaignWiseLangList(iStoreID, oTenant, iFormId);
                result.Data = value;
            }
            catch (Exception ex)
            {
