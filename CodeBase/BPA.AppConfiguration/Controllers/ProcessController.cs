...
namespace BPA.AppConfiguration.Controllers
{
    ...
    public class ProcessController : BaseController<ProcessController>
    {
        ...
        [HttpGet]
        [Route("GetProcessListDQA")]
        public async Task<MessageResponse<List<BEProcessInfo>>> GetProcessListDQA([FromQuery] int clientId)
        {
            var result = new MessageResponse<List<BEProcessInfo>>();
            try
            {
                var processList = await Task.Run(() => _repositoryProcess.GetProcessList(base.oUser.iUserID, clientId, "", true, oTenant));
