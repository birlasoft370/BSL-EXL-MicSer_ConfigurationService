    ...
    public class CampTermMappingController : BaseController<CampTermMappingController>
    {
        ...
        private BECampTermCodeMapping CatchRecord(CampTermMappingModel objCampTermMap)
        {
            using (BECampTermCodeMapping objInfo = new BECampTermCodeMapping())
            {
                ...
                for (int SelectCount = 0; SelectCount < objCampTermMap.GridTermCodeMapList.Count; SelectCount++)
                {
                    DataRow dr = dt.NewRow();
                    dr["TerminationId"] = Convert.ToInt16(objCampTermMap.GridTermCodeMapList[SelectCount]);
                    dr["IsProductive"] = objCampTermMap.GridTermCodeMapList[SelectCount].IsProductive;
                    dr["IsEnd"] = objCampTermMap.GridTermCodeMapList[SelectCount].IsEnd;
                    dr["Disabled"] = objCampTermMap.GridTermCodeMapList[SelectCount].Disabled;
                    dt.Rows.Add(dr);
                }
