...
namespace BPA.AppConfiguration.Controllers
{
    ...
    public class ProcessOffsController : BaseController<ProcessOffsController>
    {
        ...
        public async Task<MessageResponse<string>> AddProcessOffs([FromBody] ProcessOffsModel objProcessOff)
        {
        ...
        public async Task<MessageResponse<string>> UpdateProcessOffs([FromBody] ProcessOffsModel objProcessOff)
        {
            var result = new MessageResponse<string>();
            try
            {
        ...
        [NonAction]
        private BEProcessOff CatchRecord(ProcessOffsModel objProcessOffsModel)
        {
            ...
            for (int WeekDaysCount = 0; WeekDaysCount < objProcessOffsModel.DaysList.Count; WeekDaysCount++)
            {
                DataRow dr = dtProcessoff.NewRow();
                dr["WeekDays"] = objProcessOffsModel.DaysList[WeekDaysCount].DateName.ToString().Split(';')[0];
                dr["DISABLED"] = 0;
        ...
        [NonAction]
        private BEProcessOff CatchRecordForUpdate(ProcessOffsModel ObjProcessOff)
        {
            ...
            for (int count = 0; count < ObjProcessOff.DaysList.Count; count++)
            {

                DataRow dr = dtProcessoff.NewRow();
                dr["WeekDays"] = ObjProcessOff.DaysList[count].DateName;
                string[] sProcessoff = ObjProcessOff.DaysList[count].DateName.Split('/');
                dr["DISABLED"] = 0;
